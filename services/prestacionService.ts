...
class PrestacionService {
  ...
  async obtenerPrestacionesDelDia(userId?: string, forceRefresh: boolean = false): Promise<{
    ...
    try {
      ...
      // Si hay internet, verificar cache primero (a menos que sea force refresh)
      if (!forceRefresh) {
        const cachedData = await this.obtenerDeCache();
        if (cachedData) {
          return { ...cachedData, isFromCache: true, isOffline: false };
        }
      }
...
const { data: prestaciones, error } = await supabase.rpc('obtener_prestaciones_con_coordenadas', {
  ...
  p_fecha_fin: finDelDia.toISOString()
      ...
      console.log(prestaciones, 'prestaciones resultado RPC');
  ...
  async cerrarPrestacionConValidacion(
    ...
    try {
      ...
      // IMPORTANTE: NO guardar offline si hay conexión pero falla validación
      // Solo devolver el resultado de la validación
      return resultado;
    } catch (error) {
      console.error('Error cerrando prestación:', error);

      // Solo guardar offline si realmente no hay conexión
      const isOnline = await connectivityService.isOnline();
      if (!isOnline) {
